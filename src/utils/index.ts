...
export function getBrowserLang() {
  ...
  if (["cn", "zh", "zh-cn"].includes(browserLang.toLowerCase())) {
    defaultBrowserLang = LanguagesEnum.ZH_CN;
  } else {
    defaultBrowserLang = LanguagesEnum.EN_US;
  }
...
/**
 * @description: 构建树选择器结构
 * @param {Array} options 原始的树结构
 * @return {Array} 新的树结构
 */
export const buildTreeOptions = ({ options, label = "name", value = "id" }) => {
  return options.map(item => {
    return {
      ...item,
      value: item[value],
      label: item[label],
      children: item.children ? buildTreeOptions({ options: item.children, label, value }) : undefined
    };
  });
};
// 自定义防抖函数
